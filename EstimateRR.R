...
EstimateRR <- function(X.train,Tr.train,Y.train,A_mat.train,
  ...
  IC_mat <- A_mat.val/P_A_j*(1/mu0)*(Tr.val/e1*(Y.val-p1) +p1 - mean(p1))-
  A_mat.val/P_A_j* mu1/(mu0^2)*((1-Tr.val)/(1-e1)*(Y.val-p0)+p0-mean(p0))
  ...
  CI.upper <- subgroup_RR + CI.length
  CI.lower <- subgroup_RR - CI.length
  
  
  return(list("subgroup_RR" = subgroup_RR, "Upper_CI" = CI.upper, "Lower_CI" = CI.lower, "Standard_error"= SE))
}
