...
EstimateOR <- function(X.train,Tr.train,Y.train,A_mat.train,
  ...
  subgroup_OR <-  mu1/(1-mu1)/(mu0/(1-mu0))
  
  # Efficient influence function
  P_A_j <- colSums(A_mat)/nrow(A_mat)
  IC_mat <- A_mat.val/P_A_j*(1-mu0)/(mu0*(1-mu1)^2)*(Tr.val/e1*(Y.val-p1) +p1 - mean(p1))-
    A_mat.val/P_A_j* mu1/(mu0^2*(1-mu1))*((1-Tr.val)/(1-e1)*(Y.val-p0)+p0-mean(p0))
  ...
  for(k in  1:ncol(cov.mat)){
    ...
    SE[k] <- sqrt(cov.mat[k,k])/sqrt(length(Y))
  }
  
  CI.upper <- subgroup_OR + CI.length
  CI.lower <- subgroup_OR - CI.length
  
  
  return(list("subgroup_OR" = subgroup_OR, "Upper_CI" = CI.upper, "Lower_CI" = CI.lower, "Standard_error"= SE))
}
